...
def calculate_metrics(predicted, probabilities, labels, num_classes):
    ...
    return {
        ...
        "precision": "{:}".format(precision.item()),
        "recall": "{:}".format(recall.item()),
        "f1_score": "{:}".format(f1_score.item()),
        "auc": "{:}".format(auc.item()),
        "aupr": "{:}".format(aupr.item()),
        "macro_specificity": "{:}".format(macro_specificity.item()),
        "macro_mcc": "{:}".format(macro_mcc.item())
    }
...
best_epoch = 0
best_metric = 0
with open('Results/model_validation.csv', 'w') as f:
    f.write("Epoch,Train_Loss,Val_Loss,Train_Accuracy,Val_Accuracy,Train_Precision,Val_Precision,Train_Recall,Val_Recall,Train_F1,Val_F1,Train_AUC,Val_AUC,Train_AUPR,Val_AUPR,Train_Specificity,Val_Specificity,Train_MCC,Val_MCC\n")
    for epoch in range(num_epochs):
        ...
        train_metrics = calculate_metrics(y_pred, y_proba, y_true, num_classes)
        ...
        with torch.no_grad():
            ...
            f.write(f"{epoch+1}, {train_loss}, {val_loss}, {train_metrics['accuracy']}, {metrics['accuracy']}, {train_metrics['precision']}, {metrics['precision']}, {train_metrics['recall']}, {metrics['recall']}, {train_metrics['f1_score']}, {metrics['f1_score']}, {train_metrics['auc']}, {metrics['auc']}, {train_metrics['aupr']}, {metrics['aupr']}, {train_metrics['macro_specificity']}, {metrics['macro_specificity']}, {train_metrics['macro_mcc']}, {metrics['macro_mcc']}\n")
            f.flush()
